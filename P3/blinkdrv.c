...
static int encenderOApagarLeds(struct usb_blink *dev, unsigned char* message);
...
#define APAGAR_TODOS_LOS_LEDS		888
#define ENCENDER_SOLO_UN_LED		999

#define BOOLEAN		int
#define true		1
#define false		0

static int ledsQueQueremosEncender[] = {0, 0, 0, 0, 0, 0, 0, 0, 0};
static unsigned int coloresDeLosLeds[] = {0x000000, 0x000000, 0x000000, 0x000000, 0x000000, 0x000000, 0x000000, 0x000000};
static int index = 0;
...
static ssize_t blink_write(struct file *file, const char *user_buffer, size_t len, loff_t *off)
{
	...
	static int ledAEncender;
	static unsigned int colorDelLed;
	static char* split; // Aqui es donde vamos a ir guardando cada una de las particiones del comando
	char* kbuff = (char*)vmalloc(len);
	BOOLEAN entrarEnBucle = true;
	...
	kbuff[len] = '\0';
	printk(KERN_INFO "va a apagar.");

	message[0] = '\x05';
	message[1] = 0;
	message[3]=APAGAR_COLOR;
 	message[4]=APAGAR_COLOR;
 	message[5]=APAGAR_COLOR;

	int i = 0;
	//for(int i=0;i<NR_LEDS;i++){
	while(i < NR_LEDS){
		message[2]=i;
		if((retval = encenderOApagarLeds(dev, message)) < 0)
					goto out_error;
		++i;
	}
	...
	if(strcmp(kbuff,"") != 0){
		while((split = strsep(&kbuff, ",")) != NULL){
			if(sscanf(split, "%d:%x", &ledAEncender, &colorDelLed) == 2){
				message[2] = ledAEncender;
				message[3]=((colorDelLed>>16) & 0xff);
				message[4]=((colorDelLed>>8) & 0xff);
				message[5]=(colorDelLed & 0xff);

				if((retval = encenderOApagarLeds(dev, message)) < 0)
					goto out_error;
				//ledsQueQueremosEncender[index] = ledAEncender;
