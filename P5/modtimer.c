...
void my_work_func(struct work_struct *work)
{
	...
	printk(KERN_INFO "elems=%d",nElems);
...
static ssize_t timer_read(struct file *file, char *buff, size_t len, loff_t *offset){
	...
	printk(KERN_INFO "VA A ENTRAR");
	if(down_interruptible(&sem_list))
		return -EINTR;
	list_for_each_safe(cur_node, aux, &mylist) {
		kbuff = (char*) vmalloc(sizeof(int));
		item = list_entry(cur_node, struct list_item, links);
		ret += sprintf(kbuff,"%d\n",item->data);
		if(copy_to_user(buff,kbuff,sizeof(int)))
			goto fail;
		list_del(cur_node);
		vfree(item);
		vfree(kbuff);
	};
	printk(KERN_INFO "%s",kbuff); 
	up(&sem_list);
